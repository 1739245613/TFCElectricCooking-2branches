...
{
    private static final ResourceLocation ELECTRIC_OVEN_UID = ResourceLocation.fromNamespaceAndPath(TFCElectricCooking.MOD_ID, "electric_oven");
    private static final ResourceLocation ELECTRIC_SOUP_POT_UID = ResourceLocation.fromNamespaceAndPath(TFCElectricCooking.MOD_ID, "electric_soup_pot");
    ...
    {
        registry.registerBlockDataProvider(OvenComponentProvider.INSTANCE, ElectricOvenBlock.class);
        registry.registerBlockDataProvider(SoupPotComponentProvider.INSTANCE, ElectricSoupPotBlock.class);
    }
    ...
    private enum OvenComponentProvider implements IBlockComponentProvider, IServerDataProvider<BlockAccessor>
    {
        ...
        public void appendServerData(CompoundTag data, BlockAccessor access)
        {
            if (access.getBlockEntity() instanceof ElectricOvenBlockEntity oven)
            {
                ...
                data.putInt(TARGET_TEMPERATURE, oven.getTargetTemperature());
            }
        }
    ...
    private enum SoupPotComponentProvider implements IBlockComponentProvider, IServerDataProvider<BlockAccessor>
    {
        ...
        public void appendTooltip(ITooltip tooltip, BlockAccessor access, IPluginConfig config)
        {
            ...
            if (!serverData.isEmpty())
            {
                ...
                targetTemperature = serverData.getInt(TARGET_TEMPERATURE);
            ...
            else if (access.getBlockEntity() instanceof ElectricSoupPotBlockEntity soupPot)
            {
                ...
                targetTemperature = soupPot.getTargetTemperature();
            ...
            final float progress = hasOutput
                ? 1f
                : total > 0 ? Mth.clamp((float) progressValue / total, 0f, 1f) : 0f;
            final Component progressText = hasOutput
                ? Component.translatable("tfcelectriccooking.jade.done_short")
                : progress > 0f
                    ? Component.literal(Mth.floor(progress * 100f) + "%")
                    : Component.translatable("tfcelectriccooking.jade.not_started");
            final ProgressStyle style = IElementHelper.get().progressStyle()
                .color(hasOutput ? 0xFFB85C24 : 0xFF4CAF50, hasOutput ? 0xFFE07B39 : 0xFF86C36B)
                .textColor(0xFFFFFFFF);
            tooltip.add(IElementHelper.get().progress(progress, progressText, style, BoxStyle.getNestedBox(), false));
        }
        ...
        public void appendServerData(CompoundTag data, BlockAccessor access)
        {
            if (access.getBlockEntity() instanceof ElectricSoupPotBlockEntity soupPot)
            {
                ...
                data.putInt(TARGET_TEMPERATURE, soupPot.getTargetTemperature());
